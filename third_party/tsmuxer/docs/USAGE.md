...
In the BDMV Author vendored build, Linux font family/style lookup uses Fontconfig and rendering uses FreeType. macOS retains the upstream font-directory behavior. Explicit font files remain supported.
...
--repeat-playitems   | Repeat each main-path PlayItem sequence in the generated MPLS without duplicating the physical M2TS. 1-999; intended for long-lived HDMV menus.
