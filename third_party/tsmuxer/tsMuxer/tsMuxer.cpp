                           ...
                           const map<string, string>& params, AbstractStreamReader* codecReader)
{
    ...
    if (codecName[0] == 'V' ||
        ((codecName[0] == 'A' || (m_subMode && codecName == "S_HDMV/IGS")) && m_mainStreamIndex == -1))
    ...
    else if (codecName == "S_HDMV/IGS")
    {
        tsStreamIndex = 0x1400 + m_igsTrackCnt;
        m_igsTrackCnt++;
    }
    ...
    if (m_pcrOnVideo)
    {
        if (codecName[0] == 'V' || (m_subMode && codecName == "S_HDMV/IGS" && streamIndex == m_mainStreamIndex))
    ...
    {
        if (codecName == "S_SUP" || codecName == "S_HDMV/PGS" || codecName == "S_HDMV/IGS" || codecName == "S_TEXT/UTF8")
            m_pesType[tsStreamIndex] = PES_PRIVATE_DATA1;
        else
    ...
    else if (codecName == "V_MPEGI/ISO/VVC")
    {
        ...
        m_pmt.pidList[tsStreamIndex] = PMTStreamInfo(StreamType::SUB_IGS, tsStreamIndex, descrBuffer, descriptorLen,
                                                     codecReader, lang, isSecondary);
...
bool TSMuxer::doFlush()
{
    ...
    if (m_m2tsMode)
    {
        ...
        // A sparse subClip (especially preloaded IGS) can have its final
        // elementary-stream DTS behind the transport PCR already emitted for
        // PAT/PMT/PCR housekeeping.  Never let the final M2TS arrival-clock
        // interpolation move backward: ATS describes packet arrival order and
        // must be monotonic modulo its normal 30-bit wrap.
        if (m_lastPCR != -1)
...
void TSMuxer::writeEmptyPacketWithPCRTest(const int64_t pcrVal)
{
    ...
    *initTS = TSPacket::TS_FRAME_SYNC_BYTE;
    ...
    tsPacket->counter = (m_streamInfo[m_pmt.pcr_pid].m_tsCnt - 1) & 0x0f;  // adaptation-only packets repeat the previous continuity counter
...
{
    int bitsRest = 0;
    // --minbitrate is a read-ahead floor for the primary transport only.
    // Applying it to a Blu-ray subClip (notably preloaded IGS) pads a tiny
    // graphics stream with tens of megabytes of NULL packets and can make
    // its final M2TS ATS interpolation run backwards when the sparse subclip
    // is flushed.  The preloaded subclip is not a continuously read playback
    // path and does not need the primary stream's transport floor.
