...
static bool isPreloadedIGSubMuxer(const TSMuxer* muxer)
{
    if (!muxer)
        return false;
    const PIDListMap pidList = muxer->getPidList();
    return !pidList.empty() && std::all_of(pidList.begin(), pidList.end(), [](const auto& entry) {
...
DiskType checkBluRayMux(const char* metaFileName, int& autoChapterLen, vector<double>& customChaptersList,
                        ...
                        bool& stereoMode, std::string& isoDiskLabel, unsigned& repeatPlayItems, bool& finalStill)
{
    autoChapterLen = 0;
    stereoMode = false;
    repeatPlayItems = 1;
    ...
    while (str.length() > 0)
    {
        if (strStartWith(str, "MUXOPT"))
        {
            ...
            for (const auto& param : params)
            {
                ...
                else if (paramPair[0] == "--repeat-playitems" && paramPair.size() > 1)
                {
                    const int parsed = strToInt32(paramPair[1].c_str());
                    if (parsed < 1 || parsed > 999)
                        THROW(ERR_COMMON, "--repeat-playitems must be between 1 and 999")
                    repeatPlayItems = static_cast<unsigned>(parsed);
                }
                else if (paramPair[0] == "--final-still")
                    finalStill = true;
...
- S_HDMV/IGS        Interactive Graphics stream (Blu-ray HDMV menu format)
...
int main(int argc, char** argv)
{
    ...
    unsigned repeatPlayItems = 1;
    bool finalStill = false;
    int blankNum = 1900;
    ...
    try
    {
        ...
        DiskType dt = checkBluRayMux(argv[1], autoChapterLen, customChapterList, firstMplsOffset, firstM2tsOffset,
                                     insertBlankPL, blankNum, stereoMode, isoDiskLabel, repeatPlayItems, finalStill);
        ...
        if (muxMode)
        {
            ...
            if (dt != DiskType::NONE)
            {
                ...
                if (subMuxer && !preloadedIGSubMuxer)
                ...
                blurayHelper.createMPLSFile(mainMuxer, subMuxer, autoChapterLen, customChapterList, dt, firstMplsOffset,
                                            muxerManager.isMvcBaseViewR(), repeatPlayItems, finalStill);
