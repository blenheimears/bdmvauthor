...
int PGSStreamReader::readPacket(AVPacket& avPacket)
{
    avPacket.size = 0;
    avPacket.codec = this;
    avPacket.codecID = m_interactiveGraphics ? CODEC_S_IGS : CODEC_S_PGS;
    ...
    const bool graphicsStartCode =
        (m_curPos[0] == 'P' && m_curPos[1] == 'G') ||
        (m_interactiveGraphics && m_curPos[0] == 'I' && m_curPos[1] == 'G');
    if (graphicsStartCode)
    {
...
void PGSStreamReader::intDecodeStream(uint8_t* buffer, const size_t len)
{
    ...
    while (curPos < bufEnd)
    {
        ...
        if ((curPos[0] == 'P' && curPos[1] == 'G') ||
            (m_interactiveGraphics && curPos[0] == 'I' && curPos[1] == 'G'))
            curPos += 10;
                                       ...
                                       PriorityDataInfo* priorityData)
{
    ...
    if (!m_needRescale)
    {
        *dstBuffer++ = m_interactiveGraphics ? 'I' : 'P';
        *dstBuffer++ = 'G';
