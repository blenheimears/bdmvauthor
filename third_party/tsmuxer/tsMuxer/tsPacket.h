...
struct MPLSParser
{
    ...
    int m_m2tsOffset;
    // BDMV Author extension: an out-of-mux, preloaded Interactive Graphics
    // SubPath (SubPath_type 3) uses a distinct physical clip. -1 means absent.
    int m_preloadIGSubPathM2tsOffset;
    uint32_t m_preloadIGSubPathIN_time;
    uint32_t m_preloadIGSubPathOUT_time;
    // Number of times each main-path PlayItem sequence is emitted in the MPLS.
    // The physical clip is not duplicated; the playlist references it repeatedly.
    unsigned m_repeatPlayItems;
    // BDMV Author extension: mark the final main-path PlayItem as an infinite
    // still so non-looping menu media can finish while Interactive Graphics
    // remains available on the held final frame.
    bool m_finalStill;
   ...
   private:
    ...
    void composePlayItem(BitStreamWriter& writer, size_t outputPlayItemNum, size_t sourcePlayItemNum,
