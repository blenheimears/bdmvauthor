...
void CLPIParser::composeCPI(BitStreamWriter& writer, const bool isCPIExt)
{
    ...
    // A preloaded Interactive Graphics Sub TS has no video/audio access-point
    // stream from which to build an EP map. Its CPI section is therefore empty.
...
MPLSParser::MPLSParser()
    : PlayList_playback_type(0),
      ...
      OUT_time(0),
      m_m2tsOffset(0),
      m_preloadIGSubPathM2tsOffset(-1),
      m_preloadIGSubPathIN_time(0),
      m_preloadIGSubPathOUT_time(0),
      m_repeatPlayItems(1),
      m_finalStill(false),
...
void MPLSParser::composePlayList(BitStreamWriter& writer)
{
    ...
    writer.putBits(16, 0);  // reserved_for_future_use
    const MPLSStreamInfo& mainStreamInfo = getMainStream();
    const size_t sourcePlayItemCount = mainStreamInfo.m_index.size();
    const size_t repeatCount = std::max<size_t>(1U, m_repeatPlayItems);
    const size_t outputPlayItemCount = sourcePlayItemCount * repeatCount;
    if (outputPlayItemCount > 999U)
        THROW(ERR_COMMON, "Blu-ray playlist exceeds 999 PlayItems")
    writer.putBits(16, static_cast<unsigned>(outputPlayItemCount));  // number_of_PlayItems
    writer.putBits(16, number_of_SubPaths);                          // number_of_SubPaths
    for (size_t repeat = 0; repeat < repeatCount; ++repeat)
    {
        for (size_t sourcePlayItem = 0; sourcePlayItem < sourcePlayItemCount; ++sourcePlayItem)
        {
            const size_t outputPlayItem = repeat * sourcePlayItemCount + sourcePlayItem;
                                ...
                                const std::vector<PMTIndex>& pmtIndexList, const int type) const
{
    ...
    // Interactive Graphics presentation menus (SubPath_type 3) are asynchronous
    // preloaded paths and are intended to remain resident while the main path
    // advances/repeats. Other existing subpath uses retain historical behavior.
    writer.putBits(1, type == 3 ? 1 : 0);  // is_repeat_SubPath
    writer.putBits(8, 0);                  // reserved_for_future_use
                                    ...
                                    const std::vector<PMTIndex>& pmtIndexList) const
{
    ...
    int clipFileNum;
    if (m_preloadIGSubPathM2tsOffset >= 0 && subPath_type == 3)
    {
        clipFileNum = m_preloadIGSubPathM2tsOffset;
    }
    else
    {
        ...
        clipFileNum = fileNum + m_m2tsOffset;
    }

    const std::string clip_Information_file_name = strPadLeft(int32ToStr(clipFileNum), 5, '0');
    ...
    if (m_preloadIGSubPathM2tsOffset >= 0 && subPath_type == 3)
    {
        writer.putBits(32, m_preloadIGSubPathIN_time);
        writer.putBits(32, m_preloadIGSubPathOUT_time);
    }
    else
    {
...
void MPLSParser::composePlayItem(BitStreamWriter& writer, const size_t outputPlayItemNum,
                                 const size_t sourcePlayItemNum, const std::vector<PMTIndex>& pmtIndexList)
{
    const auto lengthPos = reinterpret_cast<uint16_t*>(writer.getBuffer() + writer.getBitsCount() / 8);
    writer.putBits(16, 0);  // length
    const int beforeCount = writer.getBitsCount() / 8;
    int fileNum = static_cast<int>(sourcePlayItemNum);
    if (isDependStreamExist)
        fileNum *= 2;
    ...
    // Existing split-file PlayItems retain tsMuxer's seamless-continuation
    // flag. A repeated cycle deliberately returns to the original clip-local
    // timestamps, so it is a new STC and must not falsely claim CC=6 time-axis
    // continuity. The preloaded IG subpath is independent of this main-path
    // connection and remains resident across the transition.
    const bool repeatedCycleStart = outputPlayItemNum >= pmtIndexList.size() && sourcePlayItemNum == 0;
    const int connection_condition = (outputPlayItemNum == 0 || repeatedCycleStart) ? 1 : 6;
    writer.putBits(4, connection_condition);
    writer.putBits(8, ref_to_STC_id);

    if (sourcePlayItemNum != 0 && !pmtIndexList[sourcePlayItemNum - 1].empty())
        writer.putBits(32, static_cast<unsigned>(pmtIndexList[sourcePlayItemNum].begin()->first) / 2);
    else
        writer.putBits(32, IN_time);

    if (sourcePlayItemNum != pmtIndexList.size() - 1 && !pmtIndexList[sourcePlayItemNum + 1].empty())
        writer.putBits(32, static_cast<unsigned>(pmtIndexList[sourcePlayItemNum + 1].begin()->first / 2));
    else
        writer.putBits(32, OUT_time);
    ...
    const size_t finalOutputPlayItem = pmtIndexList.size() * std::max<size_t>(1U, m_repeatPlayItems) - 1U;
    const bool infiniteStill = m_finalStill && outputPlayItemNum == finalOutputPlayItem;
    writer.putBits(8, infiniteStill ? 2 : 0);  // still_mode: 2 = infinite
    writer.putBits(16, 0);                     // reserved when still_mode != 1
    composeSTN_table(writer, sourcePlayItemNum, false);
    *lengthPos = my_htons(static_cast<uint16_t>(writer.getBitsCount() / 8 - beforeCount));
}
...
void MPLSParser::composeSTN_table(BitStreamWriter& writer, const size_t PlayItem_id, const bool isSSEx)
{
    ...
    if (!isSSEx)
    {
        ...
        writer.putBits(8, number_of_IG_stream_entries);
    ...
    if (!isSSEx)
    {
        for (auto& i : m_streamInfo)
        {
            if (i.stream_coding_type == StreamType::SUB_IGS)
            {
                i.composeStreamEntry(writer, PlayItem_id);
                i.composeStreamAttributes(writer);
            }
...
void MPLSStreamInfo::composeStreamAttributes(BitStreamWriter& writer)
{
    ...
    {
        // Presentation or Interactive Graphics stream
        CLPIStreamInfo::writeString(language_code, writer, 3);
        writer.putBits(8, 0);  // reserved_for_future_use
    }
    else
