...
bool BlurayHelper::createCLPIFile(TSMuxer* muxer, int clpiNum, bool doLog) const
{
    ...
    const bool subStream = muxer->isSubStream();
    const PIDListMap clipPidList = muxer->getPidList();
    const bool preloadedIG = subStream && !clipPidList.empty() &&
        std::all_of(clipPidList.begin(), clipPidList.end(), [](const auto& entry) {
            return entry.second.m_streamType == StreamType::SUB_IGS;
        });
    // A preloaded Interactive Graphics Sub TS is a SubPath, but it is not an
    // MVC dependent stream. Keeping isDependStream false prevents 3D-only
    // ProgramInfo_SS/CPI_SS extension records from leaking into its CLPI.
    clpiParser.isDependStream = subStream && !preloadedIG;
    if (preloadedIG)
        clpiParser.application_type = 5;  // Sub TS for a SubPath of Interactive Graphics presentation menu
    else if (clpiParser.isDependStream)
...
bool BlurayHelper::createMPLSFile(TSMuxer* mainMuxer, TSMuxer* subMuxer, int autoChapterLen,
                                  ...
                                  bool isMvcBaseViewR, const unsigned repeatPlayItems, const bool finalStill) const
{
    ...
    mplsParser.m_repeatPlayItems = std::max(1U, repeatPlayItems);
    mplsParser.m_finalStill = finalStill;
    ...
    if (subMuxer)
    {
        const bool preloadedIG = !pidListMVC.empty() &&
            std::all_of(pidListMVC.begin(), pidListMVC.end(), [](const auto& entry) {
                return entry.second.m_streamType == StreamType::SUB_IGS;
            });
        if (preloadedIG)
        {
            // Out-of-mux, asynchronous Interactive Graphics presentation menu.
            // Keep it in the ordinary STN table as stream_type=2 and point that
            // entry at a type-3 SubPath. This is the Blu-ray preloaded-IG model,
            // not MVC/dependent video and therefore must not emit SS extensions.
            mplsParser.number_of_SubPaths = 1;
            mplsParser.subPath_type = 3;
            mplsParser.m_preloadIGSubPathM2tsOffset = subMuxer->getFirstFileNum();
            const auto subFirstPts = subMuxer->getFirstPts();
            const auto subLastPts = subMuxer->getLastPts();
            if (!subFirstPts.empty() && !subLastPts.empty())
            {
                mplsParser.m_preloadIGSubPathIN_time = static_cast<uint32_t>(*subFirstPts.begin() / 2);
                mplsParser.m_preloadIGSubPathOUT_time = static_cast<uint32_t>(*subLastPts.rbegin() / 2);
            }
            for (const auto& [pid, si] : pidListMVC)
            {
                MPLSStreamInfo data(si);
                data.type = 2;
                mplsParser.m_streamInfo.push_back(data);
            }
        }
        else
        {
