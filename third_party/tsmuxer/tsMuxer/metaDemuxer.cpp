                                               ...
                                               const std::string& codecStreamName, const vector<MPLSPlayItem>& mplsInfo)
{
    ...
    else if (codecName == "S_HDMV/PGS" || codecName == "S_HDMV/IGS")
    {
        rez = new PGSStreamReader();
        if (codecName == "S_HDMV/IGS")
            dynamic_cast<PGSStreamReader*>(rez)->setInteractiveGraphics(true);
...
bool ContainerToReaderWrapper::openStream(int readerID, const char* streamName, int pid, const CodecInfo* codecInfo)
{
    ...
    if (codecInfo &&
        (codecInfo->codecID == CODEC_S_PGS || codecInfo->codecID == CODEC_S_IGS || codecInfo->codecID == CODEC_S_SUP || codecInfo->codecID == CODEC_S_SRT))
