...
#if defined(__linux__)
        if (FcInit() != FcTrue)
            THROW(ERR_COMMON, "Can't initialize Fontconfig");
#else
        loadFontMap();
#endif
        initialized = true;
...
#if defined(__linux__)
std::string TextSubtitlesRenderFT::resolveFontFile(const Font& font)
{
    // Preserve explicit font file paths.  Family-name lookup is delegated to
    // Fontconfig so Linux distributions (including NixOS) can use their normal
    // per-user/system font configuration instead of a hard-coded directory.
    if (!font.m_name.empty() && fileExists(font.m_name))
    {
        const auto resolved = tsmuxer::realpath(font.m_name);
        return resolved.empty() ? font.m_name : resolved;
    }

    const std::string family = font.m_name.empty() ? "sans-serif" : font.m_name;
    FcPattern* pattern = FcPatternCreate();
    if (pattern == nullptr)
        THROW(ERR_COMMON, "Fontconfig could not create a font pattern")

    const auto destroyPattern = [](FcPattern* p) {
        if (p != nullptr)
            FcPatternDestroy(p);
    };
    std::unique_ptr<FcPattern, decltype(destroyPattern)> requested(pattern, destroyPattern);

    if (FcPatternAddString(pattern, FC_FAMILY, reinterpret_cast<const FcChar8*>(family.c_str())) != FcTrue)
        THROW(ERR_COMMON, "Fontconfig could not set font family " << family)
    if (FcPatternAddInteger(pattern, FC_WEIGHT, (font.m_opts & Font::BOLD) ? FC_WEIGHT_BOLD : FC_WEIGHT_REGULAR) !=
        FcTrue)
        THROW(ERR_COMMON, "Fontconfig could not set font weight for " << family)
    if (FcPatternAddInteger(pattern, FC_SLANT, (font.m_opts & Font::ITALIC) ? FC_SLANT_ITALIC : FC_SLANT_ROMAN) !=
        FcTrue)
        THROW(ERR_COMMON, "Fontconfig could not set font slant for " << family)

    FcConfigSubstitute(nullptr, pattern, FcMatchPattern);
    FcDefaultSubstitute(pattern);
    FcResult result = FcResultNoMatch;
    FcPattern* matchedRaw = FcFontMatch(nullptr, pattern, &result);
    if (matchedRaw == nullptr || result == FcResultNoMatch)
        THROW(ERR_COMMON, "Fontconfig could not find a font matching " << family)
    std::unique_ptr<FcPattern, decltype(destroyPattern)> matched(matchedRaw, destroyPattern);

    FcChar8* file = nullptr;
    if (FcPatternGetString(matched.get(), FC_FILE, 0, &file) != FcResultMatch || file == nullptr)
        THROW(ERR_COMMON, "Fontconfig match for " << family << " has no font file")
...
void TextSubtitlesRenderFT::loadFontMap()
{
    ...
    auto addFontRoot = [&fileList](const string& root) {
        if (!root.empty())
            findFilesRecursive(root, "*.ttf", &fileList);
    };

    // BDMV Author runs on distributions such as NixOS where /usr/share/fonts
    // may not exist.  Allow the wrapper (or user) to provide one or more
    // colon-separated font roots without changing normal tsMuxer behavior.
    if (const char* extraRoots = std::getenv("TSMUXER_FONT_ROOT"))
    {
        string roots(extraRoots);
        size_t start = 0;
        while (start <= roots.size())
        {
            const size_t end = roots.find(':', start);
            addFontRoot(roots.substr(start, end == string::npos ? string::npos : end - start));
            if (end == string::npos)
                break;
            start = end + 1;
        }
    }
    addFontRoot(FONT_ROOT);
...
void TextSubtitlesRenderFT::setFont(const Font& font)
{
    if (m_font != font)
    {
        ...
        {
            // Fontconfig selects the actual face for the requested family/style.
            // FreeType then renders that exact file.  Do not emulate bold/italic
            // when Fontconfig has already performed style matching.
            modFontName = resolveFontFile(font);
            m_emulateItalic = false;
            m_emulateBold = false;
        }
#else
        string fontName = font.m_name;
        if (fontName.empty() && !m_fontNameToFile.empty())
            fontName = m_fontNameToFile.begin()->second;
