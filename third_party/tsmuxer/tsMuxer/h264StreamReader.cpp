    ...
    if (needInsTimingSEI)
    {
        ...
        if (m_lastIFrame)
        {
            if (m_lastSliceIDR)
            {
                SEIUnit sei;
                const SPSUnit *sps = m_spsMap.find(m_lastSliceSPS)->second;
                // The historical hard-coded value here was 855000 ticks (9.5 s).
                // Besides causing very slow AVC title starts on hardware players,
                // that value can exceed the delay permitted by the SPS HRD model.
                // Derive a 90%-full initial CPB delay from the declared HRD CPB
                // size/bitrate instead. For the Blu-ray default 30 Mb / 40 Mb/s
                // model this is 60750 ticks (0.675 s), matching x264's normal
                // vbv-init=0.9 behavior. When both NAL and VCL HRD are present,
                // use the tighter bound for every common schedule index.
                auto hrdDelay90k = [](const HRDParams& hrd, size_t idx) -> uint32_t {
                    if (!hrd.isPresent || idx >= hrd.bit_rate_value_minus1.size() ||
                        idx >= hrd.cpb_size_value_minus1.size())
                        return 0;
                    const long double bitrate =
                        (static_cast<long double>(hrd.bit_rate_value_minus1[idx]) + 1.0L) *
                        static_cast<long double>(1ULL << (6U + hrd.bit_rate_scale));
                    const long double cpbBits =
                        (static_cast<long double>(hrd.cpb_size_value_minus1[idx]) + 1.0L) *
                        static_cast<long double>(1ULL << (4U + hrd.cpb_size_scale));
                    if (bitrate <= 0.0L || cpbBits <= 0.0L)
                        return 0;
                    const uint64_t delay = static_cast<uint64_t>((cpbBits * 90000.0L * 0.9L) / bitrate);
                    const unsigned delayBits = hrd.initial_cpb_removal_delay_length_minus1 + 1U;
                    const uint64_t syntaxMax = delayBits >= 31U ? 0x7fffffffULL : (1ULL << delayBits) - 1ULL;
                    return static_cast<uint32_t>(std::max<uint64_t>(1ULL, std::min<uint64_t>(delay, syntaxMax)));
                };
                const size_t schedules = std::min<size_t>(32U, std::max<size_t>(
                    sps->nalHrdParams.isPresent ? sps->nalHrdParams.cpb_cnt_minus1 + 1U : 0U,
                    sps->vclHrdParams.isPresent ? sps->vclHrdParams.cpb_cnt_minus1 + 1U : 0U));
                for (size_t idx = 0; idx < schedules; ++idx)
                {
                    const uint32_t nalDelay = hrdDelay90k(sps->nalHrdParams, idx);
                    const uint32_t vclDelay = hrdDelay90k(sps->vclHrdParams, idx);
                    uint32_t delay = nalDelay != 0 && vclDelay != 0 ? std::min(nalDelay, vclDelay)
                                                                    : std::max(nalDelay, vclDelay);
                    if (delay == 0)
                        delay = 1;
                    sei.initial_cpb_removal_delay[idx] = static_cast<int>(delay);
                    sei.initial_cpb_removal_delay_offset[idx] = 0;
                }
