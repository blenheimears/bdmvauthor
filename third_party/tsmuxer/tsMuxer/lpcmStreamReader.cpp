...
static constexpr int MAX_HEADER_SIZE = DEFAULT_FILE_BLOCK_SIZE;
...
int LPCMStreamReader::convertWavToPCM(uint8_t* start, uint8_t* end)
{
    ...
    if (m_needPCMHdr)
    {
        ...
        // start_flag is not a generic "first packet" bit.  In the Blu-ray
        // LPCM syntax it denotes a Track-Start point and must correspond to a
        // Track-Start mark in the clip EP map.  tsMuxer does not author those
        // LPCM Track-Start marks, so setting this bit merely because this is
        // the first packet creates inconsistent stream/CLPI metadata.  Some
        // hardware players react by reinitializing the LPCM decoder at the
        // first audible samples, producing a loud pop.  Leave it clear unless
        // track-head metadata is actually implemented alongside it.
...
int LPCMStreamReader::decodeWaveHeader(uint8_t* buff, uint8_t* end)
{
    ...
    WAVEFORMATPCMEX* waveFormatPCMEx = nullptr;
    uint64_t fmtSize = 0;
    uint8_t* audioData = nullptr;

    if (m_headerType == LPCMHeaderType::htWAVE)
    {
        // Walk RIFF chunks instead of looking for "data" in a small fixed
        // window.  ffmpeg can legitimately preserve LIST/INFO, BEXT, iXML,
        // JUNK and other metadata between fmt and data.  Treating those bytes
        // as PCM is especially destructive here because convertWavToPCM()
        // byte-swaps them into Blu-ray LPCM samples, producing a loud pop at
        // the beginning of the authored stream.
        if (end - buff < 12)
            return NOT_ENOUGH_BUFFER;
        if (memcmp(buff + 8, "WAVE", 4) != 0)
            THROW(ERR_COMMON, "Invalid WAVE header: missing WAVE identifier")

        uint8_t* chunk = buff + 12;
        while (true)
        {
            if (end - chunk < 8)
                return NOT_ENOUGH_BUFFER;

            const uint32_t chunkSize = *reinterpret_cast<uint32_t*>(chunk + 4);
            uint8_t* chunkData = chunk + 8;
            const bool isFmt = memcmp(chunk, "fmt ", 4) == 0;
            const bool isData = memcmp(chunk, "data", 4) == 0;

            if (isData)
            {
                if (waveFormatPCMEx == nullptr)
                    THROW(ERR_COMMON, "Invalid WAVE header: data chunk precedes fmt chunk")
                m_curChunkLen = chunkSize;
                if (m_curChunkLen == 0)
            ...
            if (paddedChunkSize > static_cast<uint64_t>(end - chunkData))
                return NOT_ENOUGH_BUFFER;

            if (isFmt)
            {
                if (chunkSize < 16)
                    THROW(ERR_COMMON, "Invalid WAVE header: fmt chunk is too small")
                fmtSize = chunkSize;
                waveFormatPCMEx = reinterpret_cast<WAVEFORMATPCMEX*>(chunkData);
            }

            chunk = chunkData + paddedChunkSize;
            if (chunk - buff > MAX_HEADER_SIZE)
                THROW(ERR_COMMON, "WAVE header is too large")
        }
    }
    else
    {
        uint8_t* curPos = findSubstr("fmt ", buff, end);
        if (curPos == nullptr || curPos + sizeof(GUID) + 8 >= end)
            return NOT_ENOUGH_BUFFER;
        uint8_t* tmpPos = curPos + sizeof(GUID);
        fmtSize = *reinterpret_cast<uint64_t*>(tmpPos);
        if (fmtSize > static_cast<uint64_t>(end - curPos))
            return NOT_ENOUGH_BUFFER;
        waveFormatPCMEx = reinterpret_cast<WAVEFORMATPCMEX*>(tmpPos + 8);

        curPos += fmtSize;
        // in case there is a 'FLLR' (IPhone filler), skip it
        if (end - curPos >= 12 && curPos[0] == 'F' && curPos[1] == 'L' && curPos[2] == 'L' && curPos[3] == 'R')
        {
            curPos += 4;
            const int64_t fllrSize = *reinterpret_cast<int64_t*>(curPos);
            curPos += 4;
            if (fllrSize < 0 || end - curPos < fllrSize)
                return NOT_ENOUGH_BUFFER;
            curPos += fllrSize;
        }

        uint8_t* const searchStart = curPos;
        uint8_t* const searchEnd = FFMIN(searchStart + MAX_HEADER_SIZE, end);
        curPos = findSubstr("data", searchStart, searchEnd);
        if (curPos == nullptr)
        {
            if (searchEnd == end && end - searchStart < MAX_HEADER_SIZE)
                return NOT_ENOUGH_BUFFER;
            return 0;
        }
    ...
    {
        if (fmtSize < sizeof(WAVEFORMATPCMEX))
            THROW(ERR_COMMON, "Invalid extensible WAVE header: fmt chunk is too small")
    ...
    {
        // standard PCM format has no channel mask; preserve the long-standing
        // default layouts used by tsMuxer for multichannel WAV input.
        if (m_channels > 2)
        {
            if (m_channels == 3 || m_channels == 6 || m_channels == 8)
                m_lfeExists = true;
        }
    }
    else
    ...
    return static_cast<int>(audioData - buff);
}
