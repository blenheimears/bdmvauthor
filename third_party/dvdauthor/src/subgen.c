...
static void muxnext(bool eoinput)
  {
    ...
    while (newsti)
      ...
      /* wait for correct time to insert sub, leave time for vpts to occur */
        ...
        if (duegts < 0)
            duegts = 0;
        if (domux && (uint64_t)duegts > lastgts && !eoinput)
            break; /* not yet time */
        cursti = newsti;
        if (debug > 1)
          {
                    ...
                    for (i = 0; i < cursti->numbuttons; i++)
                      {
                        ...
                        snprintf(nm1, sizeof nm1, "%d", i ? i : cursti->numbuttons);
                        snprintf(nm2, sizeof nm2, "%d", (i + 1 != cursti->numbuttons) ? (i + 2) : 1);
...
l_01ba:
            if (show_progress) /* show progress */
              {
                if (lps % 1024 * 1024 * 10 < (uint64_t)secsize)
