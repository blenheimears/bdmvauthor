...
static void readpalette(struct pgc *p,const char *fname)
{
    ...
    for( i=0; i<16; i++ ) {
        ...
        if (fscanf(h.h, "%x", &pcolor) != 1) {
            fprintf(stderr, "ERR:  Palette file %s does not contain 16 valid colors\n", fname);
            varied_close(h);
            exit(1);
        }
        if( rgbf ) {
...
{
    char *localized = localize_filename(f);
    source_set_filename(curvob, localized);
    free(localized);
}
