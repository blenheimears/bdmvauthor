...
void init_locale()
  /* does locale initialization and obtains the default character set. */
  {
...
#if defined(_WIN32)
    {
      ...
      if (codepage != 0)
        snprintf(default_charset_buf, sizeof default_charset_buf, "CP%u", codepage);
      else
...
char * locale_decode
  ...
  /* not actually used anywhere */
  {
    ...
    resultnext = result;
    if (iconv(from_locale, (char **)&localestr, &inlen, &resultnext, &outlen) == (size_t)-1)
      {
        fprintf
          (
            stderr,
...
static const named_color predefined_color_names[] =
