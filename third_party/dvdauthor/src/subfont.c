...
static char *get_config_path(const char *filename)
    ...
    null, then just the config directory is returned. */
  {
    ...
    if ((homedir = getenv("HOME")) == NULL)
      ...
      {
        size_t i, imax = 0;
        const size_t exedir_len = GetModuleFileNameA(NULL, exedir, (DWORD)sizeof exedir);
        for (i = 0; i < exedir_len; i++)
            if (exedir[i] == '\\')
              {
...
//  fprintf(stderr, "glyph generated\n");
    ...
    if (glyph_width > max_bitmap_width)
      {
...
int done_freetype()
  /* called when finished with the FreeType library. */
  {
    ...
    const int err = FT_Done_FreeType(library);
    if (err)
      {
