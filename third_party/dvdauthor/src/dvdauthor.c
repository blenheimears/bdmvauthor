...
static const char * const vratedesc[16] = /* descriptions of frame-rate codes */
  {
    "0x0",
...
static unsigned int buildtimehelper(int64_t num, int64_t denom)
...
unsigned int buildtimeeven(const struct vobgroup *va, int64_t num)
    ...
    player. */
  {
    const int rc = getratecode(va);
    return
        buildtimehelper(num, ratedenom[rc]);
  } /*buildtimeeven*/
...
static void read_ifo_sector(FILE * const h, unsigned char * const buf, const char * const ifo, const char * const sector_name)
  /* Read exactly one DVD logical sector.  A short IFO must not leave stale
     bytes in the shared buffer and continue as though the sector were valid. */
  {
    const size_t got = fread(buf, 1, 2048, h);
    if (got != 2048)
      {
        fprintf(stderr, "ERR:  cannot read complete %s sector from %s (%zu/2048 bytes)\n", sector_name, ifo, got);
        fclose(h);
        exit(1);
      } /*if*/
  } /*read_ifo_sector*/
...
static void ScanIfo(struct toc_summary *ts, const char *ifo)
    ...
    into *ts for inclusion in the VMG. */
  {
    ...
    read_ifo_sector(h, buf, ifo, "VTS_PTT_SRPT"); // VTS_PTT_SRPT is 2nd sector
...
static void deletedir(const char * fbase)
  ...
  {
    static char dirname[1000], subname[1000 + 1 + 12 + 1];
    DIR  * subdir;
