...
/* uses ImageMagick/GraphicsMagick to read image s from s->fname. */
{
    ...
    for( y=0; y<im->rows; y++ ) {
        ...
        for( x=0; x<im->columns; x++ ) {
            ...
            putpixel(s, (int)(y * (png_uint_32)s->width + x), &p);
        }
    }
    DestroyImage(im);
...
/* uses libpng to read image s from s->fname. */
{
    ...
    if (fread(pnghead, 1, sizeof pnghead, fp) != sizeof pnghead) {
        fprintf(stderr, "ERR:  File %s is too short to be a png\n", s->fname);
        fclose(fp);
        return -1;
    }
    ...
    if( width>MAXX || height>MAXY ) {
        fprintf(stderr,"ERR:  PNG %s is too big: %lux%lu\n", s->fname, (unsigned long)width, (unsigned long)height);
...
static bool pickbuttongroups(stinfo *s, int ng, int useimg)
    ...
    whether to look at the pixels in s->img in addition to s->hlt and s->sel. */
  {
    ...
    assert(!useimg || (s->img.width >= 0 && s->img.height >= 0
        && s->xd <= (unsigned int)s->img.width && s->yd <= (unsigned int)s->img.height));
    assert(s->hlt.width >= 0 && s->hlt.height >= 0
        && s->xd <= (unsigned int)s->hlt.width && s->yd <= (unsigned int)s->hlt.height);
    assert(s->sel.width >= 0 && s->sel.height >= 0
        && s->xd <= (unsigned int)s->sel.width && s->yd <= (unsigned int)s->sel.height);
...
static bool scanvline(stinfo *s,unsigned char *v,rectangle *r,int x,int d)
{
    ...
    for( j=1; j<=s->outlinewidth; j++ ) {
        ...
        if( x < 0 || x >= (int)s->xd )
            return false;
        for( i=MAX(r->y0-j,0); i < MIN(r->y1 + j, (int)s->yd); i++ )
            if( v[i*s->xd+x] )
                return true;
    }
    return false;
...
static bool scanhline(stinfo *s,unsigned char *v,rectangle *r,int y,int d)
{
    ...
    for( j=1; j<=s->outlinewidth; j++ ) {
        ...
        if( y < 0 || y >= (int)s->yd )
            return false;
        for( i=MAX(r->x0-j,0); i < MIN(r->x1 + j, (int)s->xd); i++ )
            if( v[y*s->xd+i] )
                return true;
    }
    return false;
...
{
    const int width = (int)s->xd;
    const int height = (int)s->yd;
    const size_t pixels = (size_t)width * (size_t)height;
    unsigned char *visitmask = malloc(pixels);
    size_t pixel;
    int i,x,y;
    rectangle *rs=0;
    int numr=0;
    ...
    for (pixel = 0; pixel < pixels; pixel++)
        visitmask[pixel] = (s->hlt.pal[s->hlt.img[pixel]].a || s->sel.pal[s->sel.img[pixel]].a) ? 1 : 0;
    for (y = 0; y < height; y++)
        for (x = 0; x < width; x++)
            if (visitmask[y * width + x]) {
                rectangle r;
                bool didwork;
