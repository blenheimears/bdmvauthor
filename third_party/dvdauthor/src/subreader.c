    ...
    while (l)
      {
        ...
        base = FRIBIDI_PAR_ON; /* request order-neutral */
...
subtitle_elt *sub_read_line_pjs(subtitle_elt *current)
  {
    ...
    if (!sub_fgets(line, LINE_LEN))
        return NULL;
    if (sscanf(line, "%lu,%lu,\"%[^\"]",  &current->start, &current->end, text) < 3)
        return ERR;
...
subtitle_elt *sub_read_line_aqt(subtitle_elt *current)
  {
    ...
    while (true)
      ...
      // try to locate next subtitle
        if (!sub_fgets(line, LINE_LEN))
            return NULL;
        if (sscanf(line, "-->> %lu", &current->start) == 1)
            break;
      } /*while*/
#ifdef USE_SORTSUB
                            ...
                            if (sscanf(&line1[delta], "%*d:%d", &minutes))
                              {
                                if
                                  ...
                                  {
                                    sscanf(&line1[delta], "%*d:%*d:%*d.%u", &units);
                                  }
                                else
                                  {
                                    hours = 0;
                                    sscanf(&line1[delta], "%d:%d.%u", &minutes, &seconds, &units);
                                    minutes *= inverter;
                                  } /*if*/
                              }
                            else
                              {
                                hours = minutes = 0;
                                sscanf(&line1[delta], "%d.%u", &seconds, &units);
                                seconds *= inverter;
                              } /*if*/
                            jacoShift =
                                    (
