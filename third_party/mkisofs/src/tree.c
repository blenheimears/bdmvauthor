...
#define	LARGE_EXTENT	MAX_EXTENT & ~(off_t)2047L
	...
	if (s_entry->size > MAX_EXTENT) {
		...
		if ((Llong)s_entry->size > ((Llong)190 * (Llong)0x3FFFF800UL)) {
#ifndef	EOVERFLOW
...
init_fstatbuf()
{
	...
	if (fstatbuf.st_ctime == 0) {
		...
		(void) stat_set_ansecs(&fstatbuf, current_time.tv_usec);
		fstatbuf.st_mtime = current_time.tv_sec;
		(void) stat_set_mnsecs(&fstatbuf, current_time.tv_usec);
		fstatbuf.st_atime = current_time.tv_sec;
		(void) stat_set_cnsecs(&fstatbuf, current_time.tv_usec);
	}
}
