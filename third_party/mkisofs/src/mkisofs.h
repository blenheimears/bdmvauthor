...
#ifndef APPLE_HYB
/* BDMV Author: UDF retains dormant Macintosh metadata interfaces even
 * without HFS hybrid support.  Keep only the data shape those paths need. */
typedef struct bdmvauthor_hfs_file_stub {
	unsigned char type[4];
	unsigned char creator[4];
	unsigned long rsize;
} bdmvauthor_hfs_file_stub;
typedef struct hfsdirent {
	char *name;
	unsigned short fdflags;
	union { bdmvauthor_hfs_file_stub file; } u;
} hfsdirent;
#endif
...
#else
	/* UDF references these pointers for optional Mac metadata. They remain NULL. */
	struct directory_entry *assoc;
	hfsdirent	*hfs_ent;
	int		hfs_type;
#endif	/* APPLE_HYB */
#ifdef SORTING
