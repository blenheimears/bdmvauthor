	...
	struct eltorito_boot_descriptor	*boot_desc;
{
	...
	boot_desc->type[0] = 0;
	memcpy(boot_desc->id, ISO_STANDARD_ID, sizeof (boot_desc->id));
	...
	for (current_boot_entry = first_boot_entry, offset = sizeof (valid_desc);
		 ...
		 */
		if ((size_t)offset >= (SECTOR_SIZE - sizeof (boot_desc_record))) {
			comerrno(EX_BAD,
	...
	struct eltorito_boot_entry_info *boot_entry;
{
	...
	/* Compute checksum of boot image, sans 64 bytes */
		...
		if (lseek(bootimage, (off_t)8, SEEK_SET) == (off_t)-1) {
			int err = geterrno();
			(void) close(bootimage);
			comerrno(err,
			    _("Cannot seek in boot image file '%s'.\n"),
			    de->whole_name);
		}
		...
		{
			const char *bp = (const char *)&bi_table;
			size_t left = sizeof (bi_table);
			while (left != 0) {
				ssize_t n = write(bootimage, bp, left);
				if (n < 0 && geterrno() == EINTR)
					continue;
				if (n <= 0) {
					int err = n < 0 ? geterrno() : EIO;
					(void) close(bootimage);
					comerrno(err,
					    _("Cannot update boot image file '%s'.\n"),
					    de->whole_name);
				}
				bp += n;
				left -= (size_t)n;
		...
		if (close(bootimage) < 0)
			comerr(_("Cannot close boot image file '%s'.\n"),
			    de->whole_name);
	}
} /* fill_boot_desc(... */
