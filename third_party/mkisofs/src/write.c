	...
	BOOL			isnest;
{
	...
	while (dpnt) {
		...
		if ((dvd_aud_vid_flag & DVD_SPEC_VIDEO) && root == dpnt->parent &&
			...
			if ((size_t)maxlen > sizeof (dvd_path))
				maxlen = (int)sizeof (dvd_path);
...
generate_path_tables()
{
	...
	do {
		...
		for (j = 1; j < next_path_index; j++) {
			if (pathlist[j]->path_index != (unsigned int)j) {
				pathlist[j]->path_index = (unsigned int)j;
				fix++;
			}
		}
	} while (fix);
...
extern	ldate		modification_date;
	...
	memcpy(vol_desc.id, ISO_STANDARD_ID, sizeof (vol_desc.id));
	...
	FILE	*outfile;
{
	...
	memcpy(evol_desc.id, ISO_STANDARD_ID, sizeof (evol_desc.id));
