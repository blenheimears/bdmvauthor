...
/*
 * mingw-w64 UCRT GCC distinguishes the Microsoft and GNU printf dialects.
 * cdrtools uses C99 length modifiers such as %zu; use the GNU dialect on
 * native MinGW GCC just as mingw-w64's UCRT stdio declarations do.  Clang
 * intentionally keeps __printf__, matching mingw-w64's Clang headers.
 */
#if defined(__MINGW32__) && !defined(__clang__)
#define	__printflike__(fmtarg, firstvararg) \
		__attribute__((__format__(__gnu_printf__, fmtarg, firstvararg)))
#define	__printf0like__(fmtarg, firstvararg) \
		__attribute__((__format__(__gnu_printf__, fmtarg, firstvararg)))
#define	__scanflike__(fmtarg, firstvararg) \
		__attribute__((__format__(__gnu_scanf__, fmtarg, firstvararg)))
#else
#define	__printflike__(fmtarg, firstvararg) \
...
#endif /* native MinGW GCC vs generic compiler */
